...
class VAE_model(nn.Module):
    ...
    def train_model(self, data, training_parameters):
        ...
        # Modified data splits to allow for a test set
        if training_parameters['use_test_set']:
            test_set_pct = training_parameters['test_set_pct']
            validation_set_pct = training_parameters['validation_set_pct']
            train_set_pct = 1.0 - test_set_pct - validation_set_pct

            # Check that the percentages sum to 1 or less
            if train_set_pct <= 0:
                raise ValueError("Invalid split percentages. Ensure that train_set_pct > 0.")

            # First split: train+val and test
            x_temp, x_test, weights_temp, weights_test = train_test_split(
                data.one_hot_encoding,
                data.weights,
                test_size=test_set_pct,
                random_state=self.random_seed
            )
            val_relative_pct = validation_set_pct / (train_set_pct + validation_set_pct)
        ...
        if training_parameters['use_test_set']:
            # Load the best model to test performance
            best_model_path = os.path.join(training_parameters['model_checkpoint_location'], self.model_name + "_best")
            self.load(model_checkpoint=best_model_path)
            self.eval()
            with torch.no_grad():
                x_test_tensor = torch.tensor(x_test, dtype=self.dtype).to(self.device)
                test_neg_ELBO, test_BCE, test_KLD_latent, test_KLD_global_parameters = self.test_model(
                    x_test_tensor, weights_test, training_parameters['batch_size']
                )
            
            # Log results
            progress_test = "\t\t\t|Test Set Evaluation: Negative ELBO : {0:.3f}, BCE: {1:.3f}, KLD_latent: {2:.3f}, KLD_decoder_params_norm: {3:.3f} |".format(
                test_neg_ELBO, test_BCE, test_KLD_latent, test_KLD_global_parameters)
            print(progress_test)
            if training_parameters['log_training_info']:
                with open(filename, "a") as logs:
                    logs.write(progress_test + "\n")
            # No need to set back to train mode as training should be finished
    ...
    def load(self, model_checkpoint):
        '''Loads the best version of the model and sets parameters accordingly'''
        checkpoint = torch.load(model_checkpoint, map_location=self.device)
        self.load_state_dict(checkpoint['model_state_dict'])
        self.encoder_parameters = checkpoint['encoder_parameters']
        self.decoder_parameters = checkpoint['decoder_parameters']
