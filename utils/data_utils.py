...
class MSA_processing:
    def __init__(self,
        ...
        # Track which sequences are removed
    ...
    def gen_alignment(self):
        ...
        if self.preprocess_MSA:
            ...
            excluded_seq_indices = np.where(~seq_below_threshold)[0]
            for idx in excluded_seq_indices:
                seq_name = msa_df.index[idx]
                self.excluded_sequences.append(seq_name)
                self.exclusion_reasons.append('High fraction of gaps in sequence')
            ...
            for seq_name, sequence in self.seq_name_to_sequence.items():
                for letter in sequence:
                    if letter not in alphabet_set and letter != "-":
                        ...
                        # Log the exclusion reason
                        self.excluded_sequences.append(seq_name)
                        self.exclusion_reasons.append('Indeterminate amino acid in focus columns')
                        break
        ...
        if self.excluded_sequences:
            excluded_df = pd.DataFrame({
                'sequence_name': self.excluded_sequences,
                'reason': self.exclusion_reasons
            })
            excluded_df.to_csv('excluded_sequences.csv', index=False)
            print(f"Excluded sequences saved to 'excluded_sequences.csv'")
        else:
            print("No sequences were excluded during preprocessing.")
